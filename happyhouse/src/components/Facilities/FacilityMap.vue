...
export default {
  ...
  methods: {
    ...mapActions('facility', ['setItemList', 'resetFacilityState']),
    initMap() {
      ...
      const self = this;
      // 주소로 좌표를 검색합니다
      geocoder.addressSearch(this.dong, function(result, status) {
        // 정상적으로 검색이 완료됐으면
        if (status === kakao.maps.services.Status.OK) {
          ...
          var infowindow = new kakao.maps.InfoWindow({
            content:
              '<div style="width:150px;text-align:center;padding:6px 0;">' + self.dong + '</div>',
          });
      ...
      this.places = new kakao.maps.services.Places(this.map); // 장소 검색 객체
    ...
    searchPlaces() {
      ...
      // 카테 고리에 해당하는 검색
      this.places.categorySearch(this.currCategory, this.placesSearchCB, { useMapBounds: true });
    },
